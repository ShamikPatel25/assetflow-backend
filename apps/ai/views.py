...
class AIRiskAssessmentView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        # We manually check if they have HR or Org Admin roles
        if getattr(request.user, "role", None) not in ["HR_MANAGER", "ORGANIZATION_ADMIN"]:
            return Response({"detail": "You do not have permission to use AI tools."}, status=403)
