...
class IncidentViewSet(CRUDViewSet):
    ...
    def change_status(self, request, pk=None):
        ...
        if current_status in (Incident.Status.RESOLVED, Incident.Status.CLOSED):
            raise AFValidationError(f"Cannot change status of a {current_status.lower()} incident.")
        ...
        if role == "EMPLOYEE":
            # Employees can only close an OPEN incident they reported
            employee_profile = getattr(user, "employee_profile", None)
            if incident.reported_by != employee_profile:
                raise AFValidationError("You do not have permission to change the status of this incident.")
            
            if current_status != Incident.Status.OPEN or new_status != Incident.Status.CLOSED:
                raise AFValidationError("Employees can only transition OPEN incidents to CLOSED.")
        else:
            ...
            if current_status == Incident.Status.OPEN:
                if new_status not in (Incident.Status.IN_PROGRESS, Incident.Status.CLOSED):
                    raise AFValidationError("OPEN incidents can only transition to IN_PROGRESS or CLOSED.")
            elif current_status == Incident.Status.IN_PROGRESS:
                if new_status not in (Incident.Status.RESOLVED, Incident.Status.CLOSED):
                    raise AFValidationError("IN_PROGRESS incidents can only transition to RESOLVED or CLOSED.")
    ...
    def bulk_status_update(self, request):
        ...
        for inc_id in incident_ids:
            ...
            if current_status in (Incident.Status.RESOLVED, Incident.Status.CLOSED):
                errors.append({"id": inc_id_str, "error": f"Cannot change status of a {current_status.lower()} incident."})
                continue
            ...
            if role == "EMPLOYEE":
                if incident.reported_by != employee_profile:
                    errors.append({"id": inc_id_str, "error": "You do not have permission to change the status of this incident."})
                    has_error = True
                elif current_status != Incident.Status.OPEN or new_status != Incident.Status.CLOSED:
                    errors.append({"id": inc_id_str, "error": "Employees can only transition OPEN incidents to CLOSED."})
                    has_error = True
            else:
                if current_status == Incident.Status.OPEN:
                    if new_status not in (Incident.Status.IN_PROGRESS, Incident.Status.CLOSED):
                        errors.append({"id": inc_id_str, "error": "OPEN incidents can only transition to IN_PROGRESS or CLOSED."})
                        has_error = True
                elif current_status == Incident.Status.IN_PROGRESS:
                    if new_status not in (Incident.Status.RESOLVED, Incident.Status.CLOSED):
                        errors.append({"id": inc_id_str, "error": "IN_PROGRESS incidents can only transition to RESOLVED or CLOSED."})
                        has_error = True
